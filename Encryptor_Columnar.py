...
Decrypt and encrypt columnar cyphers with Encryptor.
"""
...
# Columnar Cipher
def columnar(message, key, alphabet):
    output = []
    
    n = len([*key]) # number of columns
    l = len([*message]) # number of letters
    
    height = math.ceil(l/n) # Number of letters per collumn
    r = l%n # Extra cells
    
    # word key is turned into numbers
    numkey = [*key]
    ticker = 1
    for i in range(len(alphabet)): #itterate through alphabet
        for j in range(n): 
            letter = [*key][j]
    ...
    for i in range(n):
        ...
        for j in range(height):
            index = i + j
            column.append(message[index])
        if i >= n-r: #last row business
            column[height] = ' ' #populate last row with spaces
            
        table[i] = column
        
        for i in range(n):
            collumn = table[i]
            for letter in collumn:
                output.append(letter)
    
    return ''.join(output)  # Join the output list into a string

# Invert key for easy reading
def invert(key):
    key = str(key)

# Decrypt message  
def read(message, key, alphabet):
    decrypted = columnar(message, -key, alphabet)  # Use negative key for decryption
    return decrypted

# Encrypt message    
def write(message, key, alphabet): 
    encrypted = columnar(message, key, alphabet)
    return encrypted
